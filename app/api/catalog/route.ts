...
export async function GET() {
  try {
    ...
    console.error('[catalog] Falha ao carregar catálogo:', error)
    return NextResponse.json({ error: 'Não foi possível carregar o catálogo no Neon.' }, { status: 503 })
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    console.error('[catalog] Falha ao persistir catálogo:', error)
    return NextResponse.json({ error: 'Não foi possível salvar no Neon.' }, { status: 503 })
  }
}
